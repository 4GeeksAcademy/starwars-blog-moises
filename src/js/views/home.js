...
export const Home = ({ setFavorites, favorites }) => {
          ...
          <div className="card mx-2" style={{ width: "18rem", flexShrink: 0 }} key={item.uid}>
            ...
            <div className="card-body d-flex flex-column justify-content-between">
              ...
              <div className="mt-3">
                ...
                <Link to={`/info${type.toLowerCase()}/${item.uid}`} className="btn btn-secondary w-100">
                  Info
                </Link>
              </div>
            </div>
  ...
  return (
    <div className="container">
      {renderCarousel("People", data.people, "People")}
      {renderCarousel("Planets", data.planets, "Planets")}
      {renderCarousel("Vehicles", data.vehicles, "Vehicles")}
    </div>
  );
};
